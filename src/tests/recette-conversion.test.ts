...
describe("normaliserVersKg — jonction unique avec computeRecette", () => {
  ...
  it("mode % : recompose les kg depuis % brut + masse de lot", () => {
  ...
  it("mode % sans masse de lot : refuse de convertir", () => {
    const etat: EtatCalculatrice = {
      masseLotKg: null,
      unitMode: "pct",
      pas: 0.5,
    ...
    expect(() => normaliserVersKg(etat)).toThrow(/masse de lot/i);
  });
