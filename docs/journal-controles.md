...
|---|---|---|---|
| 2026-09-24 | (aucun) | **Lot C — regroupement entre parenthèses (PRO-QHS-313 §2.1) : mesuré, pas de contrôle créé.** Arômes naturels et huiles essentielles sont regroupés dans 100 % des listes (0 séparé sur 822 listes Excel et 315 BAT lisibles). Morceaux et pétales séparés : 48 listes Excel, qui s'expliquent — noms latins des tisanes de plantes (19), « morceaux » qui ne sont pas des fruits comme vanille ou gingembre (17), séparés par d'autres ingrédients où l'ordre décroissant interdit le regroupement (7), et 5 « pétales de X, pétales de fleurs » où la fleur nommée caractérise le produit (TA6022 rose, TA5086 bruyère, TR2462 coquelicot). Un contrôle ne signalerait que des étiquettes conformes et en vente. La règle « liste en minuscule » n'appelle pas non plus de contrôle (majuscule initiale permise). | 822 listes Excel, 315 BAT lisibles. |
| 2026-09-24 | 16.2, 16.3, 16.4, 2.5 | **D3 — face-à-face fiche / attendu, et « Corriger la fiche » sur place.** 16.2 : poids net de la fiche face à ce qu'annonce le chiffre de conditionnement ; 16.3 : une ligne par élément du Gencode en écart (dans le Gencode / attendu) ; 16.4 : le produit face à celui qui porte le même Gencode. Le contrôle nomme le champ en cause (poids net, Gencode, liste Excel du 2.5) et la carte propose de le corriger, pré-rempli. Une seule action serveur relit ce champ sur le constat recalculé — le navigateur n'envoie que la valeur — et garde avant / après au journal d'audit. « BAT à refaire » n'est plus proposé sur un point calculé sur la fiche seule (16.x). | 394 produits, aucun verdict changé. À corriger avec tableau et correction : 16.2 : 10, 16.3 : 12, 16.4 : 24 ; 2.5 : 37 corrigeables. `TA6052` 16.3 : « Numéro d'article — 613 — 605 » ; 16.4 : « TA6052 — aussi porté par TA6132 ». Chemin testé sur TO2062 (même valeur, avant = après = 60). |
| 2026-09-24 | 14.1, 6.2, 13.1 | **D2 — face-à-face « Mesuré sur le BAT / Exigé ».** Une carte à corriger sur une mesure affiche un tableau : l'élément, la valeur mesurée (en rouge, avec police et corps), le minimum exigé, et une ligne de contexte (surface de la face, grammage). Seules les lignes sous le seuil sont listées ; les autres sont comptées (« 1 autre élément conforme »). Millimètres à 3 décimales : à 2, 0,896 mm s'affichait « 0,9 mm » face à « ≥ 0,9 mm ». L'Eurofeuille à la taille dérogatoire (à vérifier) a aussi son tableau. Structure générique `FaceAFace`, réutilisable pour D3. | 394 produits, aucun verdict changé ; les 189 cartes à corriger ont leur tableau (14.1 : 76, 6.2 : 42, 13.1 : 71). `TA6052` 14.1 : « Liste d'ingrédients — 0,885 mm — ≥ 0,9 mm ». `TUTJ0716` 6.2 : « « 50 » — 1,427 mm — ≥ 2 mm ». `TA6192` 13.1 : « 12,78 × 8,52 mm — ≥ 13,5 × 9 mm ». |
| 2026-09-24 | 13.3 | **Lot B — autres labels de PRO-QHS-313 v2 §11.2.** Trois logos ajoutés au robot visuel : **Max Havelaar** et **Fair for Life** (présents → « à vérifier » : pas mis en avant sur les produits finis JDG, hors poches et négoce), **Meilleur produit Bio** (le modèle lit l'année : année en cours → conforme, autre année → non conforme, illisible → à vérifier). Mention **« nouveauté / nouvelle recette / nouvelle formule »** lue dans le texte du BAT : ≤ 6 mois après la date de mise en marché → conforme, au-delà → non conforme, sans date → fiche à compléter. Tous se taisent quand le logo ou la mention est absent : aucune ligne ajoutée aux cartes. | 394 BAT : 0 mention « nouveauté » (« Nouvelle-Zélande », « de nouveaux thés » ignorés) ; aucun texte MH, FFL ou Meilleur Bio. Appel vision réel sur TA6942 : nouveau format lu, 809 jetons, aucun constat ajouté. La date de mise en marché est vide sur les 824 produits (l'Excel ne la porte pas). |
| 2026-09-24 | Tous les points lus sur le BAT | **D1 — une carte ne dit que les constats du niveau le plus grave.** Sur une carte à corriger, seuls les écarts prouvés s'affichent ; sur une carte à vérifier, seules les réserves. Les constats conformes (« étoiles présentes ») et les réserves (« même champ visuel, à confirmer ») collés à un écart disparaissent de la carte et de ses repères ; ils restent dans les preuves, comptent pour le verdict et reparaissent quand l'écart est réglé. | 394 produits, aucun verdict changé. Cartes ouvertes raccourcies : 6.2 42/42 (−253 caractères), 1.4 251 (−83), 2.4 2, 5.2 1. `TUTJ0716` 6.2 : « « 50 » … chiffres 1,427 mm, seuil 2 mm. Non conforme. », seul. |
...
- **13.3 « justifiés par la matière première »** : aucun contrôle ne vérifie encore qu'un label
  imprimé (WFTO, Fairtrade, Elephant Friendly…) correspond aux labels de la fiche. Savourez
